...
function integrate(simp::synTermSimParameters, param::synapticTerminalParameters)
    ...
    for int_step = 2:nsteps
        ...
        # integrate m 
        mNa1 = m_infNa1[1] + (mNa1 - m_infNa1)*exp(-(dt/taumNa(V_prev1)));
        mK1  = m_infK1[1] + (mK1 - m_infK1)*exp(-(dt/taumNa(V_prev1)));

        # integrate h
        hNa1 = m_infNa1[1] + (hNa1 - m_infNa1)*exp(-(dt/tauhNa(V_prev1)));

        # compute effective conductances 
        g[1] = mNa1*mNa1*mNa1*hNa1; 
        g[2] = mK1*mK1*mK1*mK1;  
        g[3] = 0; 
        g[4] = 0; 
        g[5] = 0; 
        ...
        V_inf1::Float64 = sum(g[1]*E_Na + g[2]*E_K)/sigma_g;
