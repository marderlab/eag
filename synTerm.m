	...
	methods 
		...
		function updatePlots(self,parameters)
			...
			% update xolotl objects
      			...
      			self.xolotl_obj(i).C2.f = parameters.f;

      			% update conductances
      			self.xolotl_obj(i).C1.NaV.gbar = parameters.gNa1;
      			self.xolotl_obj(i).C2.NaV.gbar = parameters.gNa2;
      			self.xolotl_obj(i).C1.Kd.gbar = parameters.gK1;
      			self.xolotl_obj(i).C2.Kd.gbar = parameters.gK2;
      			self.xolotl_obj(i).C2.CaT.gbar = parameters.gCa2;
            ...
            function configureXolotl(self)
                  ...
                  x.addCompartment('C0',-70,p.Ca_in,self.Cm,self.A,p.f,self.Ca_out,p.Ca_in,self.tau_Ca);
                  x.V_clamp = V_clamp; % only the first compartment can be clamped 

                  x.addCompartment('C1',-70,p.Ca_in,self.Cm,self.A,p.f,self.Ca_out,p.Ca_in,self.tau_Ca);
                  x.addConductance('C1','prinz/NaV',p.gNa2,self.E_Na);
                  x.addConductance('C1','prinz/Kd',p.gK1,self.E_K);
                  x.addConductance('C1','EAGwt',p.gEAG1,self.E_K);
                  x.addConductance('C1','Leak',p.gLeak,self.E_leak);

                  x.addCompartment('C2',-70,p.Ca_in,self.Cm,self.A,p.f,self.Ca_out,p.Ca_in,self.tau_Ca);
                  x.addConductance('C2','prinz/NaV',p.gNa2,self.E_Na);
                  x.addConductance('C2','prinz/Kd',p.gK2,self.E_K);
                  x.addConductance('C2','EAGwt',p.gEAG2,self.E_K);
                  x.addConductance('C2','prinz/CaT',p.gCa2,20);
                  x.addConductance('C2','Leak',p.gLeak,self.E_leak)

                  x.addSynapse('Elec','C0','C1',.05); 
                  x.addSynapse('Elec','C0','C2',.05); 

                  x.addSynapse('Elec','C1','C2',.05); 
                  x.addSynapse('Elec','C2','C1',.05); 

                  % make another object for the mutated channels
                  x2 = xolotl;
                  x2.dt = self.dt;
                  x2.t_end = self.t_end;
                  x2.addCompartment('C0',-70,p.Ca_in,self.Cm,self.A,p.f,self.Ca_out,p.Ca_in,self.tau_Ca);
                  x2.V_clamp = V_clamp; % only the first compartment can be clamped 

                  x2.addCompartment('C1',-70,p.Ca_in,self.Cm,self.A,p.f,self.Ca_out,p.Ca_in,self.tau_Ca);
                  x2.addConductance('C1','prinz/NaV',p.gNa2,self.E_Na);
                  x2.addConductance('C1','prinz/Kd',p.gK1,self.E_K);
                  x2.addConductance('C1','EAGmut',p.gEAG1,self.E_K);
                  x2.addConductance('C1','Leak',p.gLeak,self.E_leak);

                  x2.addCompartment('C2',-70,p.Ca_in,self.Cm,self.A,p.f,self.Ca_out,p.Ca_in,self.tau_Ca);
                  x2.addConductance('C2','prinz/NaV',p.gNa2,self.E_Na);
                  x2.addConductance('C2','prinz/Kd',p.gK2,self.E_K);
                  x2.addConductance('C2','EAGmut',p.gEAG2,self.E_K);
                  x2.addConductance('C2','prinz/CaT',p.gCa2,20);
                  x2.addConductance('C2','Leak',p.gLeak,self.E_leak)

                  x2.addSynapse('Elec','C0','C1',.05); 
                  x2.addSynapse('Elec','C0','C2',.05); 

                  x2.addSynapse('Elec','C1','C2',.05); 
                  x2.addSynapse('Elec','C2','C1',.05); 
